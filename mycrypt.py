...
contname = "soical media"
contpassword = "qwerty"
dicti = {
    1: ["gmail", "esstee.gmail.com", "----", "Hello, Google!1!", "first"],
    2: ["gmail", "essteegaming.gmail.com", "00000", "Hello, Google!2!", "sec"]
}
"""
"""
cryedun, cryedpwsalt = encrypt_pw(usern, password)  # sign up
with open(f"{cryedun}.key", "wb") as f:
    f.write(cryedpwsalt)
"""

"""
matchun, filen = matchingun(usern)  # sign in
if matchun:  # signing
    if signin(filen, password)[0]:
        print("Signed in!")
    else:
...
"""
cryedun, cryedpwsalt, cryeddict = encrypt_dict(usern+">-<"+contname, contpassword, dicti)
with open(f"{cryedun}.key", "wb") as f:
    f.write(cryedpwsalt + b"\n" + cryeddict)
"""
"""
matchun, filen = matchingun(usern + ">-<" + contname)  # sign in
if matchun:  # signing
    signinbool, dicti = signin(filen, contpassword, True)
    if signinbool:
        print(dicti)
        print("Signed in!")
    else:
...
print(decryedfilenames("anangamer"))
print(decryedfilenames("keremk"))"""
"""
for enfn, defn in zip(decryedfilenames("keremk"),
                          filter(lambda x: "keremk" in x, decryedfilenames())):
    print(enfn, "\t", defn)"""
